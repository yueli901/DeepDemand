...
def get_lsoa_vector(rec):
    ...
    if pop_level_key and isinstance(rec.get('population'), dict):
        parts.extend(float(x) for x in rec['population'].get(pop_level_key, []))

    # 2) employment
    emp_level_key = DATA.get('employment_level', 'lv3')
    if emp_level_key and isinstance(rec.get('employment'), dict):
...
def load_gt():
    ...
    Intersect with edges that have >0 valid OD pairs (if enabled).
    Normalize using Scaler and return:
      - edge_to_gt: {edge_id: normalized_value}
      - scaler: fitted Scaler on raw volumes
    Also prints descriptive statistics of raw GT before normalization.
    """
    # ---- Load GT JSON ----
    with open("data/traffic_volume/" + DATA["gt"], "r") as f:
        gt_data = json.load(f)

    # ---- Filter by OD availability ----
    if DATA["remove_zero_OD"]:
        with open("data/subgraphs/od_pairs_stats.json", "r") as f:
            stats_raw = json.load(f)
        valid_ids = {eid for eid, cnt in stats_raw.items() if int(cnt) > 0}
    else:
        valid_ids = {eid for eid in gt_data.keys()}

    # ---- Optional: remove motorway_link / trunk_link ----
    if DATA["remove_link"]:
        with open("data/traffic_volume/edge_to_highway_type.json", "r") as f:
            edge_to_highway = json.load(f)
        LINK_TAGS = {"motorway_link", "trunk_link"}
        valid_ids = {
            eid for eid in valid_ids
            if edge_to_highway.get(eid, "").lower() not in LINK_TAGS
        }

    # ---- Final filtered GT entries ----
    ...
    # ---- Extract raw volumes to compute stats ----
    _, raw_volumes = zip(*filtered_items)
    raw_volumes_np = np.array(raw_volumes, dtype=float)

    # ---- Print descriptive statistics ----
    print("\n=== GT Descriptive Statistics (raw) ===")
    print(f"Min     : {raw_volumes_np.min():.3f}")
    print(f"Max     : {raw_volumes_np.max():.3f}")
    print(f"Mean    : {raw_volumes_np.mean():.3f}")
    print(f"Median  : {np.median(raw_volumes_np):.3f}")
    print(f"Std     : {raw_volumes_np.std():.3f}")
    print("======================================\n")

    # ---- Normalization (optional) ----
    traffic_volume_tensor = torch.tensor(raw_volumes_np, dtype=torch.float32, device=TRAINING['device'])
    ...
    # ---- Build mapping {edge_id: value} ----
    edge_ids = [eid for eid, _ in filtered_items]
    edge_to_gt = {
        eid: float(val)
        for eid, val in zip(edge_ids, traffic_volume_tensor.cpu().numpy())
    }
